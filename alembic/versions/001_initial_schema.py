...
def upgrade() -> None:
    ...
    if 'tenants' not in existing_tables:
        op.create_table(
            ...
            sa.Column('plan', postgresql.ENUM('FREE', 'BASIC', 'PRO', 'ENTERPRISE', name='tenantplan', create_type=False), nullable=False, server_default='FREE'),
    ...
    if 'users' not in existing_tables:
        op.create_table(
            ...
            sa.Column('role', postgresql.ENUM('ADMIN', 'MANAGER', 'OPERATOR', 'MOBILE_USER', 'authenticated', name='userrole', create_type=False), nullable=False, server_default='OPERATOR'),
            sa.Column('is_active', sa.Boolean(), nullable=False, server_default='true'),
            sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    ...
    if 'manifests' not in existing_tables:
        op.create_table(
            ...
            sa.Column('carrier', postgresql.ENUM('DELHIVERY', 'EKART', 'SHADOWFAX', 'BLUEDART', 'AMAZON_SHIPPING', name='carrier', create_type=False), nullable=False),
            sa.Column('flow_type', postgresql.ENUM('DISPATCH', 'RETURN', name='flowtype', create_type=False), nullable=False),
            sa.Column('status', postgresql.ENUM('OPEN', 'CLOSED', name='manifeststatus', create_type=False), nullable=False, server_default='OPEN'),
        ...
        op.execute("""
            CREATE UNIQUE INDEX IF NOT EXISTS ix_manifest_open_unique 
    ...
    if 'scan_events' not in existing_tables:
        op.create_table(
            ...
            sa.Column('barcode_type', postgresql.ENUM('QR', 'CODE128', 'CODE39', 'EAN13', 'UNKNOWN', name='barcodetype', create_type=False), nullable=False, server_default='UNKNOWN'),
            ...
            sa.Column('sync_status', postgresql.ENUM('PENDING', 'SYNCED', 'FAILED', name='syncstatus', create_type=False), nullable=False, server_default='SYNCED'),
        )
